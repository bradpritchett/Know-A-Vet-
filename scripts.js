zipSub = (event) => {
	...
	for (var i = 0; inputElements[i]; ++i) {
		if (inputElements[i].checked) {
			checkedValues.push(inputElements[i].name);
			mtcLinks.push(inputElements[i].dataset.url);
		}
	}
	console.log(zip, checkedValues, mtcLinks);

	var googleapi = "AIzaSyAmQVw2-HXIAZqWBy2gMj66xd9OBzNyxsA";
	
	// queryVa(zip);
	// vetsDotGov = `http://veterans.${state.long}.gov/`;

	
	const auntBertha = buildAuntBerta(zip, checkedValues);
	const queryVa = buildVa(capstate, checkedValues);
	var elem = document.querySelector('#zip-results');
	elem.style.height = "800px";
	location.href = "#zip-results";
};

buildAuntBerta = (zip, checkedValues) => {
	console.log("zips ", zip)
	const terms = checkedValues;
	const urls = [];
	for (var i = 0; i < checkedValues.length; i++) {
		const url =`https://www.auntbertha.com/search/text?term=${checkedValues[i]}&postal=${zip}`;
		urls.push(url);
	}
	console.log(urls)
	return urls;
...
buildVa = (state, checkedValues) => {
	
	$.ajax({
		type: "GET",
		url: `https://sandbox-api.va.gov/services/va_facilities/v0/facilities?state=${state}&type=vet_center`,
