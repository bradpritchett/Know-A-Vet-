...
zipSub = (event) => {
	...
	let street = event.target[0].value;
	let city = event.target[1].value;
	let cityCap = city.charAt(0).toUpperCase() + city.slice(1);
	street = street.split(' ')
	var streetAd = street.join('%20')
	console.log(streetAd)
	let zip = event.target[2].value;
	let state = getState(zip);
	let term = event.target[3].value;
	console.log(event)

	googleapi = "AIzaSyAmQVw2-HXIAZqWBy2gMj66xd9OBzNyxsA";
	vaapi = "sdxDpoSvw9CDZkiOKRhKt3Y5CAeShqyy";
	queryVa(vaapi, streetAd, cityCap, state.code, zip);
	// vetsDotGov = `http://veterans.${state.long}.gov/`;

	auntBertha = `https://www.auntbertha.com/search/text?term=${term}&postal=${zip}&language=en`;
...
queryVa = (a, b, c, d, e) => {
	$.ajax({
		type: 'GET',
		url: `https://sandbox-api.va.gov/services/va_facilities/v0/nearby?street_address=${b}&city=${c}&state=${d}&zip=${e}&drive_time=90`,
		headers: { 'apikey': a }
	}).then(function (response) {
		queryVaById(response);
	})
